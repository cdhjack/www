<?php /* Smarty version Smarty-3.1.5, created on 2017-07-28 16:33:12
         compiled from "D:\www\www.rainbow.com//templates/agent\owner_form.tpl" */ ?>
<?php /*%%SmartyHeaderCode:147295959b7aec4b417-02816807%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    'e2039b4da3161d086c64524d98adb3f10bdb51cb' => 
    array (
      0 => 'D:\\www\\www.rainbow.com//templates/agent\\owner_form.tpl',
      1 => 1501223570,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '147295959b7aec4b417-02816807',
  'function' => 
  array (
  ),
  'version' => 'Smarty-3.1.5',
  'unifunc' => 'content_5959b7aeca92a',
  'variables' => 
  array (
    'site_url' => 0,
  ),
...
<?php if ($_valid && !is_callable('content_5959b7aeca92a')) {function content_5959b7aeca92a($_smarty_tpl) {?><?php echo $_smarty_tpl->getSubTemplate ("header.tpl", $_smarty_tpl->cache_id, $_smarty_tpl->compile_id, null, null, array(), 0);?>
...
<div class="agent-public-header">
    ...
    <div class="headline">添加房主</div>
</div>
    
<div class="agent-add">
    <form id="form_owner" method="post" action="#" enctype="multipart/form-data"> 
        <p><label>房主手机号：</label><input type="text" name="mobile"  id="mobile" onblur="checkInput(this.id)" value=""/></p>
        <span id="form-mobile-error" class="error"></span>
        <p><label>房主名：</label><input type="text" id="nickname" name="nickname" onblur="checkInput(this.id)" value=""/></p>
        <span id="form-nickname-error" class="error"></span>
        <p><label>身份证：</label><input type="text" name="identity_1" id="identity_1" onblur="checkInput(this.id)" value=""/></p>
        ...
        <input type="button" class="btn" value="确认添加" id="button-owner-1"/>
    </form>
</div><!--/agent-add-->
<script type="text/javascript">
//手机帐号检测是否已添加过该代理商下的房主
...
function checkButtonOwnerForm(){
 ...
 //添加房主异步提交表单
$('#button-owner-1').bind('click', function() { 
...
agent/ownerAction.php',
		...
		beforeSend: function() {
			$('#button-owner-1').attr('disabled', true);
			//$('#button-owner-1').before('<div class="attention"><img src="view/image/loading.gif" alt="" /> 请等待！</div>');
		},
		complete: function() {
			$('#button-owner-1').attr('disabled', false);
		},
		success: function(data) {
			if(data.REV == 1){
				alert(data.MSG);
				window.location.href = '<?php echo $_smarty_tpl->tpl_vars['site_url']->value;?>
agent/owner_list.php';
			}
			else{
				//alert(data.MSG);
