...
switch($act){
	case 'addpay':
		...
		//获取用户姓名和余额
		$sql = "SELECT id,rname,ye FROM user WHERE id='".$id."'";
		...
		if($updateYeValue<0){
			$res['info'] = '用户'.$coachInfo['rname'].'余额操作后为负值!';
			echo json_encode($res);exit;
		}
		$result = $db->query("INSERT INTO paydetail(`uid`,`utype`,`optype`,`money`,`idesc`,`datetime`,`adduser`) VALUES('".$id."',0,'".$optype."','".$money."','".$idesc."','".$addtime."','".($admin->u_name)."');");
		$result2 = $db->query("UPDATE user SET ye='".$updateYeValue."'  WHERE id='".$id."'");
		if($result && $result2){
			$res['msg'] = 1;
			$res['info'] = '用户'.$coachInfo['rname'].(($optype==1)?'充值':'扣款').'￥'.$money.'成功!';
		}
		else{
			$res['info'] = '用户'.$coachInfo['rname'].(($optype==1)?'充值':'扣款').'￥'.$money.'失败!';
		}
		echo json_encode($res);exit;
		break;
	default:
