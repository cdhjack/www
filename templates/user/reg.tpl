...
<div class="pwd-public-header">
    <a href="{*$site_url*}index.html" class="return"></a>
    <div class="headline">注册</div>
    <a href="{*$site_url*}user/login.php" class="ctrul">登录</a>
</div>
...
$('#button-reg-1').bind('click', function() {
	...
	$.ajax({
		url: '{*$site_url*}user/regAction.php',
		type: 'post',
		dataType: 'json',
		...
		success: function(data) {
			if(data.REV == 1){
				alert(data.MSG);
				window.location.href = '{*$site_url*}member/index.php';
			}
			else{
...
$('#mobile').bind('blur', function() {
	...
	var mobile_value = $('#mobile').val();
	$.ajax({
		url: '{*$site_url*}user/checkLoginName.php?act=reg',
		type: 'post',
		dataType: 'json',
		data:{"mobile":mobile_value},
		beforeSend: function() {
...
$('#mobile-button-1').bind('click', function() {
	...
	var mobile_value = $('#mobile').val();
	$.ajax({
		url: '{*$site_url*}user/smsAction.php?act=reg',
		type: 'post',
		dataType: 'json',
		data:{"mobile":mobile_value},
		success: function(data) {
