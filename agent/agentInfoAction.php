...
if($tx_base_64){	
	...
	//创建代理商子目录
	$file_path = $file_path . 'agent' . DIRECTORY_SEPARATOR;
	if (!file_exists($file_path)) {
		@mkdir($file_path);
	}
	//创建代理商id子目录
	$file_path = $file_path . $member_agent_id . DIRECTORY_SEPARATOR;
	if (!file_exists($file_path)) {
		@mkdir($file_path);
	}
	//开始写图片文件	
	if(!file_put_contents($file_path.$filename,$tmp_data)){				
		...
		$tx_pic = 'uploadfile/agent/'.$member_agent_id.'/'.$filename;
		$query1 = $db->query("UPDATE member set nickname='".$nickname."',identity='".$identity_1."',tx='".$tx_pic."' where id='".$member_agent_id."'");
	}
}
...
if($identity_2_base_64 || $wx_pay_qrcode_base_64){	
	...
	if(!empty($tmp_data) && !file_put_contents($file_path.$filename,$tmp_data)){				
		...
		$identity_pic = !empty($tmp_data)?'uploadfile/agent/'.$member_agent_id.'/'.$filename:'';
		$wx_pay_qrcode = !empty($tmp_data2)?'uploadfile/agent/'.$member_agent_id.'/'.$filename2:'';
		
		//查询该用户是否有附属表信息记录
		$r = $db->fetchRow ("select id,identity_pic,wx_pay_qrcode from member_info where member_id='".$member_agent_id."' limit 1");
		...
		if($member_info_id){
			...
			//member_info表入库
			$query2 = $db->query("INSERT INTO member_info(`member_id`,`identity_pic`,`wx_pay_qrcode`,`add_time`) VALUES('".$member_agent_id."','".$identity_pic."','".$wx_pay_qrcode."','".$AddTime."')");
		}
	}
}
...
if($wx_group_qrcode_base_64){	
	...
	if(!file_put_contents($file_path.$filename,$tmp_data)){				
		...
		$wx_group_qrcode = 'uploadfile/agent/'.$member_agent_id.'/'.$filename;
		//echo "UPDATE member_type set wx_group_qrcode='".$wx_group_qrcode."' where id='".$member_agent_id."' and member_type=3";exit;
		$query3 = $db->query("UPDATE member_type set wx_group_qrcode='".$wx_group_qrcode."' where member_id='".$member_agent_id."' and member_type=3");
	}
}
