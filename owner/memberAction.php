...
//会员类型表入库处理（判断用户是否有玩家类型记录）
$query3 = true;
$r = $db->fetchRow ("select id from member_type where member_id='".$member_id."' and member_type=1 limit 1");
...
if($member_type_id){
	...
	//member_type表入库
	$query3 = $db->query("INSERT INTO member_type(`member_id`,`member_type`,`invite_code`,`add_time`) VALUES('".$member_id."',1,'".$invite_code."','".$AddTime."')");
}


//房主与玩家关系表member_owner入库处理,房主下线数量更新
$query4 = true;
$query5 = true;
$r = $db->fetchRow ("select id from member_owner where member_id='".$member_id."' and member_owner_id='".$member_owner_id."' limit 1");
$member_owner_relation_id = isset($r['id'])?$r['id']:0;
if($member_owner_relation_id){
	//更新member_owner表
	//$query4 = $db->query("UPDATE member_owner set member_id='".$member_id."',member_owner_id='".$member_owner_id."' where id='".$member_owner_relation_id."'");
}
else{
	$AddTime = time();
	
	//member_owner表入库
	$query4 = $db->query("INSERT INTO member_owner(`member_id`,`member_owner_id`,`balance`,`add_time`) VALUES('".$member_id."','".$member_owner_id."',0,'".$AddTime."')");
	//房主下线数量更新
	$query5 = $db->query("UPDATE member_type set friend_count=friend_count+1 where member_id='".$member_owner_id."' and member_type=2");
...
if($query1 && $query2 && $query3 && $query4 && $query5){
	$result['REV'] = 1;
	$result['MSG'] = empty($passwd_1)?'恭喜,添加玩家成功!':'恭喜,添加玩家成功,该玩家的初始密码为:"'.$passwd_1.'"!';
}
else{
	/*echo 'query1:<br>';
	print_r($query1);
